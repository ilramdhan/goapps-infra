#!/usr/bin/env bash
# IAM Setup Script — runs migration + seed for a given namespace.
# Automatically uses the same image tag as the running iam-service deployment.
#
# Usage:
#   ./scripts/iam-setup.sh <namespace> [action]
#
# Examples:
...
#   ./scripts/iam-setup.sh goapps-production seed   # seed only
#
# Prerequisites:
#   - iam-service deployment must be running in the target namespace
#   - For custom admin credentials, create iam-seed-secret first (see seed-job.yaml)
...
BASE_DIR="$SCRIPT_DIR/../services/iam-service/base"

# Get image tag from running deployment
IMAGE_TAG=$(kubectl get deployment iam-service -n "$NAMESPACE" \
  -o jsonpath='{.spec.template.spec.containers[0].image}' 2>/dev/null | grep -oP ':\K.*$')

if [ -z "$IMAGE_TAG" ]; then
  echo "ERROR: Could not find iam-service deployment in namespace '$NAMESPACE'"
  echo "       Make sure iam-service is deployed and ArgoCD has synced."
  exit 1
fi
